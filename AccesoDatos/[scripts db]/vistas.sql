...
CREATE VIEW VW_GetAllTickets AS
	SELECT
		T.Id AS 'TicketID', C.Nombre + ' - ' + SC.Nombre AS 'Titulo', T.IdUsuarioCreador AS 'UsuarioCreadorID', U.Nombre, T.FechaCreacion, T.FechaActualizacion, T.Descripcion, T.IdEstado AS 'EstadoID', ET.NombreEstado
	FROM Tickets AS T
...
CREATE VIEW VW_GetTicketInfo AS
	SELECT
		T.Id, U.Id AS 'IdUsuarioCreador', U.Nombre, U.TipoUsuario, U.Correo, C.Nombre + ' - ' + SC.Nombre AS 'Titulo', T.IdEstado, ET.NombreEstado, T.FechaCreacion, T.FechaActualizacion, T.Descripcion
	FROM Tickets AS T
	INNER JOIN SubCategorias AS SC ON T.IdSubCategoria = SC.Id
	INNER JOIN Categorias AS C ON SC.IdCategoriaPadre = C.Id
	INNER JOIN Usuarios AS U ON T.IdUsuarioCreador = U.Id
